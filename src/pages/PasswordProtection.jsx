    ...
    setSavedUrls((current) => {
      if (selectedItem) {
        return current.map((item) =>
          item.id === selectedItem.id ? entry : item,
        );
      }
      return [entry, ...current];
    });
    setMessage(
      selectedItem ? "Password rule updated." : "Password protection added.",
    );
    setSelectedId(null);
    setShortUrlSlug("");
    setPassword("");
    ...
    <section className="page-content">
      ...
      <header className="page-header">
        <div>
          ...
          <p>Secure short links with a password over shared access.</p>
        </div>
        <UsageBadge count={savedUrls.length} />
      </header>
      <div className="dashboard-toolbar">
        <form className="feature-form-password" onSubmit={handleSubmit}>
          <label>
            Protected link name
            <input
              value={name}
        ...
        <div className="admin-table-password">
          <div className="admin-row-password admin-header-password">
            <span>Name</span>
            <span>Slug</span>
            <span>Action</span>
          </div>
          {savedUrls.map((item) => (
            <div key={item.id} className="admin-row-password">
